...
fps = 30.0  # fps
width = round(cap.get(cv2.CAP_PROP_FRAME_WIDTH))  # 영상 가로 길이
height = round(cap.get(cv2.CAP_PROP_FRAME_HEIGHT))  # 영상 세로 길이

fourcc = cv2.VideoWriter_fourcc(*'XVID') # 코덱 지정
writer = cv2.VideoWriter('output4.avi', fourcc, fps, (width, height))
